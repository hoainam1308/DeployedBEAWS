...
const Order = require('../schemas/order'); // Assuming you have an Order model defined
...
router.get('/vnpay_return', async function (req, res, next) {
    ...
    if(secureHash === signed){
        ...
        if (!order) return res.render('success', { code: '01' });
...
router.get('/vnpay_ipn',  function (req, res, next) {
    ...
    const amount = Number(vnp_Params['vnp_Amount']) / 100; // VNPay *100
    ...
    try {
        ...
        console.error(err);
        return res.status(200).json({ RspCode: '99', Message: 'Uncaught error' });
    }
});
